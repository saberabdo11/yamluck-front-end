        ...
        <div class="sub-categories">
          <div class="categories">
            <div v-for="subCat in subCats" :key="subCat.id">
              <NuxtLink :to="`${subCat.id}`" style="color: #d1a125">
                <button class="card active" v-if="param == subCat.id">
                  {{ subCat[`title_${locale}`] }}
                </button>
                <button class="card" v-else>
                  {{ subCat[`title_${locale}`] }}
                </button>
              </NuxtLink>
            </div>
          ...
          <div class="sub-categories">
            ...
            >
              <ul style="display: flex; gap: 25px">
                <li v-for="subSubCat in subSubCats" :key="subSubCat.id">
                  <NuxtLink
                    :to="`${$route.path}/${subSubCat.id}`"
                    style="color: #d1a125"
                  >
                    {{ subSubCat[`title_${locale}`] }}
                  </NuxtLink>
                </li>
              </ul>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      favs: [],
      page: 1,
      catParam: this.$route.params.id,
      param: this.$route.params.subid,
  ...
  async fetch() {
    await this.$http
      .$get(`https://backend.yammluck.com/api/get-sub-offers/${this.param}`)
      .then((succ) => {
        this.offers = succ;
    ...
    this.subCats = await this.$http.$get(
      `https://backend.yammluck.com/api/get-subcategory/${this.$route.params.id}`
    );
    this.subSubCats = await this.$http.$get(
      `https://backend.yammluck.com/api/get-subsubcategory/${this.param}`
    );
    if (this.$auth.loggedIn) {
      this.favs = await this.$axios.$post(
        "https://backend.yammluck.com/api/favs",
        {
  ...
  mounted() {
    axios
      .get(`https://backend.yammluck.com/api/carousels/${this.catParam}`)
  ...
  methods: {
    getResults(page) {
      this.$axios
        .$get(
          `https://backend.yammluck.com/api/get-sub-offers/${this.param}?page=${page}`
        )
        .then((result) => {
          this.offers = result;
        })
